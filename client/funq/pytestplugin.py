...
def pytest_addoption(parser):  # pragma: no cover - exercised through pytest
    ...
    group.addoption('--funq-screenshot-folder', action='store',
                    ...
                    help='Folder where screenshots are stored after failures '
...
def pytest_runtest_teardown(item, nextitem):  # pragma: no cover
    del nextitem
    state = getattr(item.config, '_funq_state', None)
    if state is not None:
...
class _FunqPytestState:
    ...
    def on_test_start(self, item):
        self._log_message(f"Starting test `{self._describe_item(item)}`")

    def on_test_finish(self, item):
        self._log_message(f"Ending test `{self._describe_item(item)}`")
    ...
    def _build_options(self):
        ...
        opts.funq_trace_tests_encoding = pytest_options(
            'funq_trace_tests_encoding')
